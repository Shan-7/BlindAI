...
def translate_to_hindi_and_speech(text):
    try:
        ...
        if not text.strip():
            print("Input text is empty. Translation not performed.")
            return

        # Translate text to Hindi
        res = EngtoHindi(text)
        translated_text = res.convert
...
def capture_and_ocr():
    ...
    while True:
        ...
        elif key == ord('k'):  # Press 'k' to perform speech-to-text in English, translate to Hindi, and speak
            try:
                # Initialize recognizer class (for recognizing the speech)
                r = sr.Recognizer()
                ...
                with sr.Microphone() as source:
                    ...
                    # Using google to recognize English audio
                    ...
                    hindi_tts.save("translated_hindi_output.mp3")
                    os.system("afplay translated_hindi_output.mp3")  # macOS specific command to play the audio
